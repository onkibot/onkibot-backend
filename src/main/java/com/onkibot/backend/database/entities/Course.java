...
public class Course {
   ...
   *
   * <p>This method will also check if a {@link User} attends the course if <code>user</code> is set
   * and not null.
   *
   ...
   * @param user The {@link User} entity we want to check if attends the {@link Course} with ID
   *     <code>courseId</code>.
   * @throws CourseNotFoundException If a {@link Course} with the <code>courseId</code> is not
   *     found, or if the <code>user</code> argument is not attending the {@link Course} with ID
   *     <code>courseId</code> is set and not null.
