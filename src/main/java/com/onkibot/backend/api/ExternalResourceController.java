...
public class ExternalResourceController {
    ...
    ExternalResourceModel get(
            @PathVariable int courseId, @PathVariable int categoryId, @PathVariable int resourceId, @PathVariable int externalResourceId) {
        ExternalResource externalResource = this.assertCourseCategoryExternalResource(courseId, categoryId, resourceId, externalResourceId);
        return new ExternalResourceModel(externalResource);
    }

    @RequestMapping(method = RequestMethod.GET)
    Collection<ExternalResourceModel> getAll(@PathVariable int courseId, @PathVariable int categoryId, @PathVariable int resourceId) {
        Resource resource = this.assertCourseCategoryResource(courseId, categoryId, resourceId);
        return resource.getExternalResources().stream().map(ExternalResourceModel::new).collect(Collectors.toList());
    }
    ...
    private ExternalResource assertCourseCategoryExternalResource(int courseId, int categoryId, int resourceId, int externalResourceId) {
